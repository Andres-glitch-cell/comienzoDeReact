...
const DEFAULT_IMAGES: ImageItem[] = [
  // ... tus imágenes por defecto (las mantengo iguales)
  ...
  // ... resto omitido por brevedad, pega el tuyo completo
];
...
export default function DomeGallery({
  ...
  const lockScroll = useCallback(() => {
    if (scrollLockedRef.current || !document.body) return;
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      radius = clamp(radius, minRadius, maxRadius === Infinity ? Number.MAX_SAFE_INTEGER : maxRadius);
      ...
      // Ajuste de enlarge si ya está abierto
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        if (hasCustomSize) {
          // Temporal para calcular tamaño custom
          const tempDiv = document.createElement("div");
          tempDiv.style.position = "absolute";
          tempDiv.style.width = openedImageWidth;
          tempDiv.style.height = openedImageHeight;
  ...
  }, [fit, fitBasis, minRadius, maxRadius, padFactor, overlayBlurColor, grayscale, imageBorderRadius, openedImageBorderRadius, openedImageWidth, openedImageHeight, applyTransform]);
  ...
  const openItemFromElement = useCallback((el: HTMLElement) => {
    if (openingRef.current || !rootRef.current) return;
    ...
    // Usamos const para evitar warnings de mutación innecesaria
    const rawRotY = -(parentY + globalY) % 360;
    const rotY = rawRotY < -180 ? rawRotY + 360 : rawRotY;
    ...
    // Forzar reflow para asegurar que las posiciones se calculen bien
    ...
    const rawSrc = parent.dataset.src || imgEl?.src || "";
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.style.position = "absolute";
      ...
      animatingOverlay.style.zIndex = "9999";
      animatingOverlay.style.borderRadius = "var(--enlarge-radius, 32px)";
      animatingOverlay.style.overflow = "hidden";
      animatingOverlay.style.boxShadow = "0 10px 30px rgba(0,0,0,.35)";
            ...
            {items.map((it, i) => (
              ...
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.alt || "Abrir imagen"}
                  onClick={onTileClick}
